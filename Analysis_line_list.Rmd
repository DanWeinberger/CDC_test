...
,
 ...
 first.beta<-paste0("f(beta1,", names(X)[1], ",model='iid',hyper = param.beta,", "values=c(", paste(1:ncol(X), collapse=',') ,"))" )
    ...
    next.beta.f<-function( covar.index, Xnames){ 
      paste0("f(beta",covar.index, names(X[-1])[covar.index], ",copy='beta1', fixed=T)")
    }
        betas.list<- pbmapply(covar.index=2:ncol(X), Xnames=colnames(X)[-1],next.beta.f,SIMPLIFY=T)
    ...
    all.betas<-paste(first.beta,next.betas , sep='+')   
          ...
          form1<- as.formula(paste0("all_cause_pre ~",  all.betas    ) )
           ...
           mod1.full = inla(form1, data = mod.df.full, 
                     control.predictor = list(compute=TRUE, link = 1), offset = ag3$log_pop,
                     family='nbinomial')
