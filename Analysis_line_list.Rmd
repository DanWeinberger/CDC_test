...
mod.mat1 <- model.matrix(~
              1 + time_scale + qtr2 + qtr3 +qtr4 +
                  ...
                  agec*(time_scale + qtr2 + qtr3 +qtr4) +
                  race_recode*(time_scale + qtr2 + qtr3 +qtr4)+
                  region*(time_scale + qtr2 + qtr3 +qtr4), data=ag3)
...
preds.inla2 <- gen_pred_interval_inla(inla_obj=mod.inla2, covar.df=ag3, mod.mat=mod.mat1 ,log.offset1=ag3$log_pop)
```
...
```{r}
 p1 <- ggplot(preds.age.race[preds.age.race$agec=='11',], aes(x=date, y=N_deaths )) +
    geom_line() +
    geom_line(aes(x=date, y=pred_median, group=agec), lty=2, col='black') +
    geom_ribbon(aes(ymin=pred_lcl, ymax=pred_ucl), alpha=0.2, col='gray') +
    ylab("Number of all-cause deaths") +
...
ag3.complete <- ag3[!is.na(ag3$sex) & !is.na(ag3$time_scale) & !is.na(ag3$race_recode) & !is.na(ag3$all_cause_pre) & !is.na(ag3$region), ]

X <- model.matrix( ~1 + time_scale + qtr2 + qtr3 +qtr4 +
                  agec + race_recode + sex, data=ag3.complete)

Z <- model.matrix( ~ -1 + agec*(time_scale + qtr2 + qtr3 +qtr4) +
                  race_recode*(time_scale + qtr2 + qtr3 +qtr4)+
                  region*(time_scale + qtr2 + qtr3 +qtr4), data=ag3.complete)

Z <- Z[,23:ncol(Z)]

formula1 = all_cause_pre ~ -1 + X + f(id.z, model='z', Z=Z)

result <- inla(formula1 , data=list(all_cause_pre=ag3.complete$all_cause_pre, id.z=as.numeric(dimnames(ag3.complete)[[1]]) ,X=X) )
