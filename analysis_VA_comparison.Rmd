...
VA data
```{r}
v1 <- read.csv('./Data/Confidential/va_aggdata_all.csv')
v1$source <- 'VA'
v1 <- v1[v1$Age.Group %in% c('25-44 years' ,'45-64 years','65-74 years','75-84 years' ,  '85 years and older'),]
v1 <- v1[,c('Week.Ending.Date', 'Age.Group','Number.of.Deaths', 'source')]
names(v1) <- c('week_end','age_group', 'all_cause', 'source')
...
v2 <- v1 %>%
  group_by(week_end,age_group,source) %>%
...
```{r}
p1 <- ggplot(c1[c1$source=='NCHS',], aes(x=week_end, y=all_cause)) +
      geom_line() +
...
p2 <- ggplot(c1[c1$source=='VA',], aes(x=week_end, y=all_cause)) +
      geom_line() +
      ylab("Number of all-cause deaths") +
      xlab("Date") +
      theme_classic() +
...
rr.ls <- lapply(mod1[[1]] , function(x){
rr.nchs <- x$NCHS$y/x$NCHS$pred 
rr.va <- x$VA$y/x$VA$pred 
date <- x$NCHS$date
cbind.data.frame(date, rr.nchs, rr.va, age_group=)
})

rr.ls <- lapply(names(rr.ls) , function(x) {
  rr.ls[[x]]$age_group=x 
  return(rr.ls[[x]])
  }
  )
