...
pop1$age_group[pop1$AGE>=85 & pop1$AGE<85] <- '85 years and older' #category 85=85+; cat 999 is all combined
...
names(pop2) <- c('age_group','state','variable','popsize')

pop2 <- pop2[,c('age_group','state','popsize')]
...
ag3 <- merge(ag3, pop2, by=c('state','age_group'))

ag3$log_pop <- log(ag3$popsize)
...
mod.inla1 <- inla(all_cause_pre ~ 
                1 + time_scale + 
                sin1 + sin2 + sin3 + cos1 +cos2+ cos3  + age_group +state ,
                offset = log_pop,
                family='poisson', data=ag3)
summary(mod.inla1)
```
...
summary.spl.85plus <- summary.spl[ages=='85 years and older']

par(mfrow=c(6,3), mar=c(2,2,1,1))
lapply(summary.spl.85plus, function(x){
  plot(x$week_end, x$all_cause, type='l', bty='l', main=unique(x$state))
